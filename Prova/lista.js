...
function ex9(){
   ...
   for(let i = 0; i<10; i++){
        nome[i] = String(prompt(`Nome do produto: `))
        cod[i] = Number(prompt(`Código: `))
        preco[i] = Number(prompt(`Preço do produto: `))

        if(cod[i]%2 == 0 || preco[i] > 1000){
            if(cod[i]%2 == 0 && preco[i] > 1000){
                novopreco = preco[i] + (preco[i]*0.20)
            }

            else if(cod[i]%2 == 0){
                novopreco = preco[i] + (preco[i]*0.15)
            }

            else{
                novopreco = preco[i] + (preco[i]*0.10)
            }

            console.log(`O produto ${nome[i]} de código ${cod[i]} aumentou de R$ ${preco[i]} para R$ ${novopreco}`)
        }
    }
}
